...
function utils.splitByChar(str,inSplitPattern)
	--print('inside splitbyChar func',str)
	outResults={}
	local theStart = 1
	local theSplitStart,theSplitEnd=string.find(str,inSplitPattern,theStart)
	--print('theSplitStart & theSplitEnd::',theSplitStart,theSplitEnd)
	while theSplitStart do
		local temp = string.sub(str,theStart,theSplitStart-1)
		--print('temp::',temp)
		table.insert(outResults,temp)
...
function utils.getNgrams(config,doc,n,pad)
	...
	local lpad_len=math.floor(missing_tokens/2)
	local rpad_len=missing_tokens-lpad_len
	--print('lpad & rpad :: ', lpad_len,rpad_len)
...
function utils.write_vocabToCSV(config,sep)
	...
	for i=1,#config.index2word do
		--print('$$$$$$$$$utils vocab word::',config.index2word[i])
		--print(config.index2word[i])
		file:write(utils.trim(config.index2word[i]))
		file:write('\n')
	end
	file:close()
	...
	-- dumping word2index and index2word 
        ...
        local file_path =directory..filename
        --local file = assert(io.open(file_path, "w"))
        torch.save(file_path,config.word2index)
        file.close()

        local filename = 'index2word.dat'
        local file_path =directory..filename
        --local file = assert(io.open(file_path, "w"))
        torch.save(file_path,config.index2word)
        file.close()
	    print('Done saving word2index & index2word dats')
